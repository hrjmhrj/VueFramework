<template>
  <div style="height: 100%;width: 100%;overflow: hidden;">
    ...
    <div style="margin-top: 15%;text-align: left;margin-left: 5%;margin-bottom: 8%;font-size: 6vw;font-weight: 600;">
      注册
    </div>
    <van-form @submit="onRegister">
      ...
      <div style="margin-left:5%;margin-top:3%;width: 91%">
        <van-button round block type="info" native-type="submit" v-if="registering">
          注册
        </van-button>
        <van-button round block type="info" disabled loading loading-text="注册中..." v-else/>
      </div>
    </van-form>
  </div>
</template>
    ...
    methods: {
      ...
      onRegister(values) {
        ...
        if (!pswd.test(this.zcmima)) {
          Notify({type: 'danger',
            duration: 4000,message: '密码最少六位最多二十位，字母或数字组合，请区分大小写!'});
          return
        };
        if (values["zcmima"] != values["querenzcmm"]) {
          Notify({type: 'danger', message: '两次输入的密码不一致，请检查。'});
          return
        }
